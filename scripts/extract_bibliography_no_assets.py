...
sin guardar imágenes ni crear carpetas de assets.

Diseñado para un proyecto con estructura ``project_name/scripts`` y
``project_name/bib`` y para recuperación "repo-native"
...
Ejemplos
--------
Procesamiento normal, sin assets:

    python scripts/extract_bibliography_no_assets.py

Forzar reprocesamiento:

    python scripts/extract_bibliography_no_assets.py --force

Usar explícitamente CUDA:

    python scripts/extract_bibliography_no_assets.py --device cuda

Activar reconocimiento LaTeX de fórmulas:

    python scripts/extract_bibliography_no_assets.py --formulas

Activar OCR para PDFs escaneados:

    python scripts/extract_bibliography_no_assets.py --ocr

Guardar además el DoclingDocument como JSON:

    python scripts/extract_bibliography_no_assets.py --json
...
def discover_default_paths() -> tuple[Path, Path]:
    """
    Detecta automáticamente las rutas del proyecto.
    ...
    También funciona si se ejecuta desde ``scripts/`` o desde cualquier
    directorio, usando la ubicación de este archivo para encontrar la raíz
    del proyecto.
    """
    ...
    # Ejecutado directamente desde bib/.
    if (cwd / "pdf").is_dir():
        input_dir = cwd / "pdf"
        output_dir = cwd / "extracted"
        return input_dir, output_dir

    # Busca bib/ desde la raíz actual o desde project_name/scripts/.
    project_roots = [cwd]

    if cwd.name.lower() in {"scripts", "bib"}:
        project_roots.append(cwd.parent)

    # Permite ejecutar el script desde cualquier directorio.
    project_roots.append(Path(__file__).resolve().parent.parent)

    for project_root in project_roots:
        for bib_name in ("bib", "Bib"):
            bib_dir = project_root / bib_name

            if (bib_dir / "pdf").is_dir():
                return bib_dir / "pdf", bib_dir / "extracted"

    # Valor por defecto coherente con project_name/{scripts,bib} aunque
    # bib/pdf todavía no exista; process_corpus mostrará el error exacto.
...
def build_converter(
    *,
    ...
    # Nunca guardamos imágenes de página ni de figuras: los PDFs
    # originales se conservan como fuente visual del corpus.
...
def convert_pdf(
    *,
    ...
    # PDF -> Markdown textual. Las imágenes se representan como
    # placeholders dentro del Markdown, sin escribir archivos binarios.
...
def print_header(
    *,
    ...
    print("Imágenes         : desactivadas (sin carpeta de assets)")
