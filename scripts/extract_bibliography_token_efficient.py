...
without persisted image assets.
...
* Uses picture/layout processing internally when needed for faithful extraction.
...
* Does not persist page images, table crops, figure crops, or any other assets.
...
* Adds compact page markers (<!-- p:N -->) to the Markdown.
...
def discover_repo_root() -> Path:
    ...
    for bib_name in ("bib", "Bib"):
        if (project_root / bib_name).is_dir():
            return project_root
    raise FileNotFoundError(
        "No pude localizar la raíz del proyecto con estructura "
        "project_name/{scripts,bib}. Ejecuta el script desde el proyecto "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "de contexto, sin guardar assets visuales."
        )
    )
    parser.add_argument("--repo", type=Path, default=None, help="Raíz de project_name.")
    parser.add_argument(
        "--paper",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        help=(
            "Escala interna para el análisis de páginas; no se guardan imágenes. "
            "Default: 3.0 (~216 DPI)."
        ),
    )
    parser.add_argument(
...
def build_fidelity_converter(
    *,
    ...
    # Needed internally for layout/table analysis. These images stay in memory
    # and are never persisted as repository assets.
    pipeline.generate_page_images = True
    pipeline.generate_picture_images = False
    pipeline.images_scale = image_scale

    # Picture classification is unnecessary when no picture assets are exported.
...
def remove_generic_picture_placeholders(text: str) -> str:
    # The PDF remains the visual source of truth. Generic placeholders only waste tokens.
...
def export_assets(
    *,
    ...
    # Kept as a defensive compatibility stub: this profile never persists
    # tables or figures, even if an old caller reaches this helper.
    return AssetStats()
    ...
    *,
    ...
    if index_path.exists():
        ...
        original = (
            ...
            "Índice compacto para recuperación. Use Markdown primero y el PDF como "
            "fuente final de verificación.\n\n"
            "## Papers\n\n"
        )
...
def process_one_pdf(
    *,
    ...
    try:
        ...
        # The PDF remains the visual source of truth. No image assets are
        # written to disk by this profile.
        print("  [ASSETS] disabled; keeping only Markdown and references")
...
def main() -> int:
    ...
    try:
        ...
        print("Persist images  : no (assets disabled)")
        print("=" * 80)
